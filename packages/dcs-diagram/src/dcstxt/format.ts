// 数据格式化, 前后缀与格式化掩码之间必须包含空格，无空格则不显示后缀或前缀
const maskRegex = /[\ ][0-9\-+#]/;
...
function processMask(mask = " #.## ") {
	const maskObj = {
		...
		integer: "",
		mask: "",
		maskHasNegativeSign: false,
		maskHasPositiveSign: false,
		prefix: "",
		separator: ",",
		suffix: "",
	};
	const len = mask.length;
	const start = getIndex(mask) + 1;
	maskObj.prefix = start > 0 ? mask.substring(0, start) : "";

	// Reverse string: not an ideal method if there are surrogate pairs
	const end = getIndex(mask.split("").reverse().join("")) + 1;
...
export function format(mask, value, options = {}) {
