...
import { Node, Pen } from '@topology/core';
...
// getPenText: Get the pen text from text.
export function getPenText(pen: Pen) {
  let txt: string;
  if (!pen.text) {
    pen.text = '';
  }
  console.log(pen.data)
  if (!pen.data || pen.data.type === undefined) {
    return pen.text
  }
  let paras = pen.data
  switch (paras.type) {
    case 'value':
      pen.text = format(paras.fmt, paras.value)
      break;
    case 'msg':
      for (let i = 1; i < paras.msgs.length; ++i) {
        if (paras.msgs[i].cond) {
          pen.text = String(paras.msgs[i].value);
          break;
        }
      }
      break;
...
export function dcsTxt(ctx: CanvasRenderingContext2D, node: Pen) {
  if (!node.text) {
    return;
  }
  if (!node.text.split) {
